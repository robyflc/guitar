...
var chords = (function(){
    ...
    var ChordBoxImage = function(name, chord, fingers, size) {
        ...
        var _foregroundBrush = '#fff';
        var _backgroundBrush = '#222';
    ...
    var ReplaceChordElements = function() {
        $('chord').each(function(i, elt){
            ...
            var variation = $(elt).attr('variation');
            var fingers = $(elt).attr('fingers');
            var size = $(elt).attr('size');
            var chord = ChordBoxImage(name, positions, fingers, size);
            var canvas = $('<canvas></canvas>').data({variation:variation}).attr({width:chord.getWidth(), height:chord.getHeight()}).insertAfter(elt);
